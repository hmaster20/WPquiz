...
function co_quiz_questions_meta_box($post) {
    ...
    wp_localize_script('jquery', 'co_quiz_admin', [
...
/**
 * JavaScript для управления вопросами и викторинами (inline, поскольку нет assets/).
 */
function co_enqueue_questions_script() {
    if (isset($_GET['post_type']) && in_array($_GET['post_type'], ['co_quiz', 'co_question']) ||
        (isset($_GET['post']) && in_array(get_post_type($_GET['post']), ['co_quiz', 'co_question']))) {
...
add_action('admin_enqueue_scripts', 'co_enqueue_questions_script');
?>