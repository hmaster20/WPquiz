...
function cot_test_questions_meta_box($post) {
    ...
    <div id="cot-test-questions">
        ...
        <select name="cot_questions[]" multiple style="width:100%;height:150px;">
            ...
            <option value="<?php echo $question->ID; ?>" <?php echo in_array($question->ID, $question_ids) ? 'selected' : ''; ?>>
        ...
        <div id="cot-new-questions-list">
            ...
            <div class="cot-new-question">
                <input type="text" name="cot_new_questions[<?php echo $index; ?>][title]" value="<?php echo esc_attr($new_question['title']); ?>" placeholder="<?php _e('Question title', 'career-orientation-test'); ?>" />
                <textarea name="cot_new_questions[<?php echo $index; ?>][content]" placeholder="<?php _e('Question description', 'career-orientation-test'); ?>"><?php echo esc_textarea($new_question['content']); ?></textarea>
                <div class="cot-new-answers">
                    <?php foreach ($new_question['answers'] as $ans_index => $answer) : ?>
                    <div class="cot-answer">
                        <input type="text" name="cot_new_questions[<?php echo $index; ?>][answers][<?php echo $ans_index; ?>][text]" value="<?php echo esc_attr($answer['text']); ?>" placeholder="<?php _e('Answer text', 'career-orientation-test'); ?>" />
                        <input type="number" name="cot_new_questions[<?php echo $index; ?>][answers][<?php echo $ans_index; ?>][weight]" value="<?php echo esc_attr($answer['weight']); ?>" placeholder="<?php _e('Weight', 'career-orientation-test'); ?>" />
                        <button type="button" class="button cot-remove-new-answer"><?php _e('Remove', 'career-orientation-test'); ?></button>
                    </div>
                    <?php endforeach; ?>
                    <button type="button" class="button cot-add-new-answer" data-question-index="<?php echo $index; ?>"><?php _e('Add Answer', 'career-orientation-test'); ?></button>
                </div>
...
function cot_save_test($post_id) {
    ...
    if (isset($_POST['cot_questions'])) {
        $question_ids = array_map('intval', $_POST['cot_questions']);
        update_post_meta($post_id, '_cot_questions', $question_ids);
    } else {
        delete_post_meta($post_id, '_cot_questions');
    }
    // Save new questions
    if (isset($_POST['cot_new_questions'])) {
        ...
        foreach ($new_questions as $new_question) {
            if (!empty($new_question['title'])) {
                ...
                if ($question_id && !empty($new_question['answers'])) {
                    $answers = array_map(function($answer) {
                        return [
                            'text' => sanitize_text_field($answer['text']),
                            'weight' => intval($answer['weight']),
    ...
    <div class="wrap">
        ...
        <?php foreach ($tests as $test) : 
            ...
            foreach ($results as $result) {
                ...
                $answer = get_post_meta($result->question_id, '_cot_answers', true)[$result->answer_id]['text'];
        ...
        <canvas id="chart-<?php echo $test->ID; ?>" width="400" height="200"></canvas>
        ...
        <script>
            jQuery(document).ready(function($) {
                var ctx = document.getElementById('chart-<?php echo $test->ID; ?>').getContext('2d');
                new Chart(ctx, {
                    type: 'bar',
                    data: {
                        labels: <?php echo json_encode(array_keys($chart_data)); ?>,
                        datasets: [
                            <?php foreach ($chart_data as $question => $answers)喧) : ?>
                            {
...
function cot_test_shortcode($atts) {
    ...
    if ($test->post_type !== 'cot_test') return __('Invalid test', 'career-orientation-test');
    ...
    ob_start();
    ?>
    <form id="cot-test-form-<?php echo $test_id; ?>" method="post">
        <input type="hidden" name="cot_test_id" value="<?php echo $test_id; ?>">
        <?php foreach ($questions as $question) : 
    ...
    <?php
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['cot_test_id']) && $_POST['cot_test_id'] == $test_id) {
        global $wpdb;
        $table_name = $wpdb->prefix . 'cot_results';
